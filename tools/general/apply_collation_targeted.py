"""
apply_collation_targeted.py
Aplica cambio de collation a Modern_Spanish_CI_AS sobre las columnas identificadas
por el SQL de diagnóstico (nullable, collation incorrecto), ejecutando en bloques
de 10 tablas con confirmación interactiva antes de cada bloque.

Ciclo por tabla:
  1. DROP  — índices afectados (DROP CONSTRAINT para PK/UNIQUE, DROP INDEX para el resto)
  2. ALTER — columnas con collation incorrecto
...
# ------------------------------------------------------------------
# SQL 1 — Lista de tablas afectadas (orden determinista)
# ------------------------------------------------------------------
SQL_TABLAS = """
...
#          Parametrizado con un IN (...) generado en Python.
# ------------------------------------------------------------------
...
def procesar_tabla(cursor, tabla, drops, alters, creates):
    ...
    for sql in drops:
        try:
            ...
            # Si el DROP falla, lo reportamos pero continuamos (puede que no existiera)
            msg = str(e)
            if "3701" in msg or "Cannot drop" in msg or "does not exist" in msg.lower():
                # Índice/constraint no existe — ignorar
                pass
            else:
    ...
    for sql in alters:
        try:
            ...
            # Intentar rehacer los drops antes de salir
            for create_sql in creates:
                if create_sql in [s for s in creates]:
                    # Recrear todos los que se dropearon
                    pass
            for drop_sql in drops_ejecutados:
                # Extraer el CREATE equivalente y ejecutarlo
                pass
            # Ejecutar todos los creates para intentar dejar la tabla íntegra
...
def run(solo_bloque=None):
    """
    solo_bloque: si se indica un número (1-based), solo ejecuta ese bloque
                 con confirmación automática (modo prueba).
    """
    ok_tablas = []
    err_tablas = []   # [(tabla, motivo)]
    skip_tablas = []
    ...
    with db_connection() as conn:
        ...
        if not tablas:
            print("No hay columnas que necesiten migración de collation. Nada que hacer.")
            return

        print(SEP)
        print(f"  Tablas con columnas a migrar: {len(tablas)}")
        print(SEP)
        for t in tablas:
            print(f"    {t}")
        ...
        for num_bloque, bloque_tablas in enumerate(bloques, start=1):
            ...
            if solo_bloque is not None and num_bloque != solo_bloque:
                ...
                continue  # saltar bloques que no son el seleccionado

            resp = "s" if solo_bloque == num_bloque else None
            if resp is None:
                resp = confirmar_bloque(num_bloque, total_bloques, bloque_tablas, n_drops, n_alters, n_creates)
            else:
                # Mostrar resumen igualmente
                print(f"\n{SEP}")
            ...
            for tabla in bloque_tablas:
                if tabla not in por_tabla:
                    # No había nada que hacer para esta tabla (ya migrada?)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Migración de collation a Modern_Spanish_CI_AS por bloques.")
    parser.add_argument("--bloque", type=int, default=None,
                        help="Ejecutar solo el bloque N (1-based). Útil para pruebas.")
    args = parser.parse_args()
    run(solo_bloque=args.bloque)
